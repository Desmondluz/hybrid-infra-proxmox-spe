...
- Modules TF site-agnostic (`terraform/modules/proxmox-vm`, `terraform/modules/network`)
- Onboarding documenté : `docs/onboarding-new-site.md`

---

## Synthèse — code livré vs déployé live

Les deux dimensions évoluent à des rythmes différents : le code, la
configuration et la documentation sont versionnés et lintables dès
maintenant ; le déploiement live (VMs réellement instanciées sur Proxmox,
services up, dashboards alimentés) suit le calendrier des follow-ups.

| Dimension              | FW1     | FW2 (en cours)         | FW3 (cible) | Final |
| ---------------------- | ------- | ---------------------- | ----------- | ----- |
| 📝 Code livré          | 60 %    | **94 %**               | 100 %       | 100 % |
| 🚀 Déployé live (S1)   | 0 %     | **≈ 30 %** (VMs S1)    | 80 %        | 100 % |
| 🚀 Déployé live (S2)   | 0 %     | 0 % (planifié P1.11)   | 80 %        | 100 % |
| 🧪 DRP exercé          | 0 %     | 0 % (planifié FW3)     | 60 %        | 100 % |

**État FW2 (snapshot live)** : voir [`docs/STATUS.md`](docs/STATUS.md) —
mis à jour à chaque jalon.

**Pourquoi cet écart est normal et assumé** :

- Les critères CIA évaluent la **qualité du code, de la doc et de
  l'architecture**. Tout est auditable hors-ligne (lint CI vert, diagrammes,
  runbooks, ADR). C'est le périmètre `📝 Code livré`.
- Le `🚀 Déployé live` est limité par les contraintes matérielles (un
  seul Proxmox physique mutualisé, virtualisation imbriquée à débloquer)
  documentées dans [`docs/STATUS.md §Blockers`](docs/STATUS.md#blockers).
- La **démo jury de 5 min** s'appuiera sur les VMs S1 déjà up (pfSense,
  bastion, NetBox, Elastic) + un `terraform plan` sur S2 + une exécution
  Ansible en check-mode pour prouver la reproductibilité.
